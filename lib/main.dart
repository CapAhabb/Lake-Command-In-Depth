...
void main() async {
  ...
  // Force landscape on startup to mimic chartplotter orientation
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const Color deepBase = Color(0xFF0D0E13); // sampled from jpeg
    const Color panel = Color(0xFF1C1521);
    const Color panelAlt = Color(0xFF131520);
    const Color accentPurple = Color(0xFF5A0454); // purple seabed
    const Color accentOrange = Color(0xFF6D3C36); // orange/brown highlight
    const Color highlight = Color(0xFFA2B599); // pale highlight
              ...
              child: Center(
                child: ClipRRect(
                  ...
                  child: SizedBox(
                    ...
                    child: Stack(
                      children: <Widget>[
                        Positioned.fill(child: CustomPaint(painter: LakeMichiganPainter())),
                        Positioned.fill(child: Opacity(opacity: 0.6, child: CustomPaint(painter: SeabedPainter()))),
                        for (final SpeciesMarkerData marker in markers)
                          Align(alignment: marker.alignment, child: _LakeMarker(marker: marker)),
                        const Align(alignment: Alignment(-0.55, -0.86), child: _MiniCompass()),
                      ],
                    ),
                  ),
                ),
                  ...
                  child: Row(
                    children: <Widget>[
                      Expanded(
                        ...
                        child: Stack(
                          children: <Widget>[
                                ...
                                children: <Widget>[
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      ...
                                      border: Border.all(color: const Color(0x26B8E8F1)), 
                                    ),
                                    child: const Icon(Icons.directions_boat, size: 28, color: Colors.white),
                                  ),
                                ],
                              ),
                            ),
                            ...
                            Positioned(
                              left: 12,
                              right: 12 + 180, // leave space for right column
                              bottom: 12,
                              child: Container(
                                height: 96,
                                decoration: BoxDecoration(
                                  color: const Color(0xDD08151B),
                                ...
                                child: Row(
                                  ...
                                  children: <Widget>[
                                    _controlItem('Species\nDensity', 0.95, active: true),
                                    _controlItem('Bait\nDensity', 0.02),
                                    _controlItem('Current', 0.0),
                                    _controlItem('Current @\nDepth', 0.0),
                                    _controlItem('Surface\nTemp', 0.0),
                                    _controlItem('Thermocline', 0.0),
                                  ],
                                ),
                              ),
                            ),
                      ...
                      Container(
                        width: 180,
                        decoration: BoxDecoration(
                          color: const Color(0xFF061A20),
                          borderRadius: const BorderRadius.only(
                            topRight: Radius.circular(28),
                            bottomRight: Radius.circular(28),
                          ),
                          border: Border(left: BorderSide(color: const Color(0xFF284451))),
                        ),
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                            Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: <Widget>[
                                const Text('LakeGuard Pro', style: TextStyle(color: Color(0xFF98ADB8), fontWeight: FontWeight.w700)),
                                Icon(Icons.signal_cellular_4_bar, color: const Color(0xFF98ADB8)),
                              ],
                            ),
                            const SizedBox(height: 10),
                            _rightMetric('Depth', '52ft'),
                            const SizedBox(height: 8),
                            _rightMetric('Water', '67.9°'),
                            const SizedBox(height: 8),
                            _rightMetric('Speed', '3.1 mph'),
                            const SizedBox(height: 8),
                            _rightMetric('GPS', '0:2'),
                            const SizedBox(height: 8),
                            Expanded(
                              child: Column(
                                ...
                                children: <Widget>[
                                  Row(
                                    mainAxisAlignment: MainAxisAlignment.center,
                                    children: <Widget>[
                                      ElevatedButton(onPressed: () {}, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFFC9500)), child: const Text('MENU')),
                                    ],
                                  ),
                                ],
                              ),
...
class LakeMichiganPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Paint lakeFill = Paint()
      ..shader = const LinearGradient(
        colors: <Color>[
          Color(0xFF5A0454), // purple
          Color(0xFF6D3C36), // orange/brown
          Color(0xFFA2B599), // pale highlight
        ],
        begin: Alignment.topCenter,
        end: Alignment.bottomCenter,
      ).createShader(rect);
...
Widget _controlItem(String label, double value, {bool active = false, VoidCallback? onTap, ValueChanged<double>? onAdjust}) {
  return Expanded(
    child: GestureDetector(
      onTap: onTap,
      onLongPress: () {
        if (onAdjust != null) {
          // show bottom sheet to adjust value
          // use context via Builder
        }
      },
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: <Widget>[
          Text(
            ...
            style: const TextStyle(color: Color(0xFFB7CBD2), fontSize: 12),
          ),
          const SizedBox(height: 6),
          SizedBox(
            width: 36,
            ...
            child: CircularProgressIndicator(
              ...
              color: active ? const Color(0xFF87D97A) : const Color(0xFF98ADB8),
          ...
          Container(
            ...
            child: const Text('AUTO', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 12)),
          ),
        ],
      ),
    ),
...
Widget _rightMetric(String title, String value) {
  return Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: <Widget>[
      Text(title, style: const TextStyle(color: Color(0xFF98ADB8), fontSize: 12)),
      const SizedBox(height: 6),
...
// Chartplotter-style landing that maps the JPEG into an interactive bezel
// Simple mock seabed painter (re-added)
class SeabedPainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    final Paint p = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = 1.0
      ..color = const Color(0x55FFB86B); // warm overlay line

    for (int i = 0; i < 6; i++) {
      final Path path = Path();
      final double yOffset = size.height * (0.18 + i * 0.11);
      path.moveTo(0, yOffset);
      for (int x = 0; x <= 12; x++) {
        final double xPos = (size.width / 12) * x;
        final double yPos = yOffset + math.sin((x + i * 1.3) * 0.8 + i) * 5.0;
...
class ChartPlotterLanding extends StatefulWidget {
  const ChartPlotterLanding({super.key, required this.markers, required this.onEnter});

  final List<SpeciesMarkerData> markers;
  final VoidCallback onEnter;

  @override
  State<ChartPlotterLanding> createState() => _ChartPlotterLandingState();
}

class _ChartPlotterLandingState extends State<ChartPlotterLanding> {
  // overlay states
  bool seabedOn = true;
  bool markersOn = true;
  bool controlsOn = true;

  // tuned defaults for phone/chartplotter appearance
  double seabedOpacity = 0.55;
  double markersOpacity = 0.90;
  double controlsOpacity = 0.85;

  Alignment boatAlignment = Alignment.center;
  late Timer _mockGpsTimer;
  double _t = 0;
  ...
  void initState() {
    ...
    _loadPrefs();
    // mock GPS movement
    _mockGpsTimer = Timer.periodic(const Duration(milliseconds: 250), (_) {
      setState(() {
        _t += 0.06;
        boatAlignment = Alignment(math.sin(_t) * 0.18, math.cos(_t * 0.8) * 0.12);
      });
    });
  }
  ...
  Future<void> _loadPrefs() async {
    try {
      ...
      setState(() {
        seabedOpacity = prefs.getDouble('seabedOpacity') ?? seabedOpacity;
        markersOpacity = prefs.getDouble('markersOpacity') ?? markersOpacity;
        controlsOpacity = prefs.getDouble('controlsOpacity') ?? controlsOpacity;
      });
    } catch (_) {}
  }

  Future<void> _savePref(String key, double value) async {
    try {
      final SharedPreferences prefs = await SharedPreferences.getInstance();
  ...
  void _showOpacityControl(String title, double value, ValueChanged<double> onChanged) {
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Slider(value: value, min: 0.0, max: 1.0, onChanged: (v) => setState(() => onChanged(v))),
              const SizedBox(height: 8),
  ...
  Widget _bezelButton({required Alignment alignment, required String label, required bool active, required VoidCallback onTap, required VoidCallback onHold}) {
    return Align(
      alignment: alignment,
      child: GestureDetector(
        onTap: onTap,
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: active ? const Color(0xFF0F3240) : const Color(0xCC08151B),
            borderRadius: BorderRadius.circular(8),
            border: Border.all(color: active ? const Color(0xFFFFD27A) : const Color(0xFF2E4A55), width: active ? 2 : 1),
            boxShadow: active
                ? const [BoxShadow(color: Color(0x33FFD27A), blurRadius: 8, offset: Offset(0, 4))]
                : const [],
          ),
          alignment: Alignment.center,
          child: Text(label, textAlign: TextAlign.center, style: TextStyle(color: active ? Colors.white : const Color(0xFF98ADB8), fontWeight: FontWeight.w800, fontSize: 13)),
        ),
      ),
    );
  }

  Widget _dialControlLocal(String label, double value, ValueChanged<double> onChanged, {bool active = false}) {
    String key;
    final String lower = label.toLowerCase();
    if (lower.contains('seabed')) {
      key = 'seabedOpacity';
    ...
    return Expanded(
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: <Widget>[
          Text(label, textAlign: TextAlign.center, style: const TextStyle(color: Color(0xFFB7CBD2), fontSize: 12)),
          const SizedBox(height: 6),
          RotaryKnob(
            size: 56, // reduced to match jpeg scale
            value: value,
            onChanged: (double v) {
              onChanged(v);
              _savePref(key, v);
          ...
          Container(
            ...
            child: Text('${(value * 100).round()}%'.padLeft(3), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800, fontSize: 12)),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Stack(
        children: <Widget>[
          // constructed chart screen (drawn, interactive)
          Positioned.fill(
            child: Container(
                 ...
                 child: ClipRRect(
                   ...
                   child: AspectRatio(
                     ...
                     child: Stack(
                       children: <Widget>[
                         Positioned.fill(child: CustomPaint(painter: LakeMichiganPainter())),
                         if (seabedOn) Positioned.fill(child: Opacity(opacity: seabedOpacity, child: CustomPaint(painter: SeabedPainter()))),
                         Positioned.fill(child: Opacity(opacity: 0.04, child: Container(color: Colors.black))),
                         for (final SpeciesMarkerData m in widget.markers) Align(alignment: m.alignment, child: _LakeMarker(marker: m)),
                         Align(alignment: boatAlignment, child: Container(width: 56, height: 56, decoration: BoxDecoration(color: Colors.white12, borderRadius: BorderRadius.circular(8), border: Border.all(color: const Color(0x33B8E8F1))), child: const Center(child: Icon(Icons.directions_boat, color: Colors.white, size: 28)),)),
                         const Align(alignment: Alignment(-0.55, -0.86), child: _MiniCompass()),
                       ],
                     ),
                   ),
          ...
          // bottom dials (replace previous static controls)
          if (controlsOn)
            Positioned(
             left: 120,
             right: 120,
             bottom: 46, // raised dials higher
             child: Container(
               height: 92,
               decoration: BoxDecoration(
                 color: const Color(0xDD08151B),
               ...
               child: Row(
                 mainAxisAlignment: MainAxisAlignment.spaceBetween,
                 children: <Widget>[
                   _dialControlLocal('Seabed', seabedOpacity, (v) => setState(() => seabedOpacity = v), active: seabedOn),
                   _dialControlLocal('Markers', markersOpacity, (v) => setState(() => markersOpacity = v), active: markersOn),
                   _dialControlLocal('Controls', controlsOpacity, (v) => setState(() => controlsOpacity = v), active: controlsOn),
                 ],
               ),
             ),
            ),

          // Bezel buttons placed to match the photo (approximate positions)
          // Left column
          _bezelButton(
            alignment: const Alignment(-0.86, -0.62),
            label: 'MENU',
            active: true,
            onTap: () => setState(() {/* menu pressed — keep for parity */}),
            onHold: () => _showOpacityControl('MENU (no opacity)', 1.0, (_) {}),
          ),
          _bezelButton(
            alignment: const Alignment(-0.86, -0.18),
            label: 'LAYER',
            active: markersOn,
            onTap: () => setState(() => markersOn = !markersOn),
            onHold: () => _showOpacityControl('Markers opacity', markersOpacity, (v) => markersOpacity = v),
          ),
          _bezelButton(
            alignment: const Alignment(-0.86, 0.18),
            label: 'SEABED',
            active: seabedOn,
            onTap: () => setState(() => seabedOn = !seabedOn),
            onHold: () => _showOpacityControl('Seabed opacity', seabedOpacity, (v) => seabedOpacity = v),
          ),
          _bezelButton(
            alignment: const Alignment(-0.86, 0.62),
            label: 'CTRL',
            active: controlsOn,
            onTap: () => setState(() => controlsOn = !controlsOn),
            onHold: () => _showOpacityControl('Controls opacity', controlsOpacity, (v) => controlsOpacity = v),
          ),
...
class RotaryKnob extends StatefulWidget {
  const RotaryKnob({super.key, required this.size, required this.value, required this.onChanged, this.active = false});
...
class _RotaryKnobState extends State<RotaryKnob> {
  late double _angle; // radians

  @override
  void initState() {
    super.initState();
    _angle = (widget.value.clamp(0.0, 1.0) * 1.6) - 0.8; // map 0..1 to -0.8..0.8 radians
  }

  void _updateFromLocal(Offset localPosition) {
    final RenderBox box = context.findRenderObject() as RenderBox;
    final Offset center = box.size.center(Offset.zero);
    final Offset offset = localPosition - center;
    final double angle = math.atan2(offset.dy, offset.dx);
    // clamp angle to -0.8..0.8
    final double clamped = angle.clamp(-0.8, 0.8);
    setState(() {
      _angle = clamped;
      final double v = ((_angle + 0.8) / 1.6).clamp(0.0, 1.0);
      widget.onChanged(v);
    });
  }
  ...
  void didUpdateWidget(covariant RotaryKnob oldWidget) {
    ...
    _angle = (widget.value.clamp(0.0, 1.0) * 1.6) - 0.8;
  }
        ...
        child: Stack(
          ...
          children: <Widget>[
            Container(
              ...
              height: widget.size,
              decoration: BoxDecoration(
                color: widget.active ? const Color(0xFF0F3240) : const Color(0xCC08151B),
                shape: BoxShape.circle,
                border: Border.all(color: const Color(0xFF2E4A55), width: widget.active ? 2 : 1),
              ),
            ),
            Transform.rotate(
              angle: _angle,
              child: Container(
                ...
                decoration: BoxDecoration(color: const Color(0xFFB7CBD2), borderRadius: BorderRadius.circular(8)),
              ),
            ),
          ],
        ),
